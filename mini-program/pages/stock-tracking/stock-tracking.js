...
Page({
  data: { stocks: [], code: '', selectedStock: null, suggestions: [], loading: true, adding: false, message: '' },
  ...
  async onAdd() { if (!this.data.selectedStock) return this.setData({ message: '请先选择搜索结果' }); if (this.data.adding) return; this.setData({ adding: true, message: '' }); try { await addTrackedStock(this.data.selectedStock.tsCode); this.setData({ code: '', selectedStock: null, suggestions: [], message: '股票已添加，正在刷新列表…' }); await this.loadStocks(); } catch (error) { this.setData({ message: error.message.includes('503') ? 'Tushare 暂不可用' : '添加股票失败，请稍后重试' }); } finally { this.setData({ adding: false }); } },
