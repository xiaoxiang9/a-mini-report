...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="main-nav">{navigation.map((item) => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => isActive ? 'nav-item active' : 'nav-item'}><span className="nav-glyph">{item.glyph}</span>{item.label}{item.label !== '总览' && <span className="nav-dot">{item.label === '每日复盘' ? 'LIVE' : 'SOON'}</span>}</NavLink>)}</nav>
