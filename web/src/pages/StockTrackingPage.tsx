...
export function StockTrackingPage() {
  ...
  useEffect(() => { void load(); }, []);
  useEffect(() => { const keyword = code.trim(); if (!keyword) { setSuggestions([]); return; } const timer = window.setTimeout(() => { searchStocks(keyword).then(setSuggestions).catch(() => setSuggestions([])); }, 300); return () => window.clearTimeout(timer); }, [code]);
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setMessage('');
    try { await addTrackedStock(code); setCode(''); setSuggestions([]); await load(); } catch (error) { setMessage(error instanceof Error && error.message.includes('503') ? 'Tushare 暂不可用，请稍后重试' : '添加失败，请选择搜索结果或输入标准股票代码'); }
  };
  const selectSuggestion = async (item: StockSearchResult) => { if (item.isTracked) { setMessage('该股票已在追踪列表中'); return; } try { await addTrackedStock(item.tsCode); setCode(''); setSuggestions([]); await load(); } catch { setMessage('添加股票失败，请稍后重试'); } };
  const remove = async (tsCode: string) => { if (!window.confirm(`确认移除 ${tsCode}？`)) return; await removeTrackedStock(tsCode); await load(); };
  return <div className="module-page page-wrap"><p className="overline">STOCK TRACKING</p><h1>个股追踪</h1><p className="module-intro">持续跟踪关注标的，所有指标标注最新交易日。</p><form className="stock-add-form" onSubmit={submit}><div className="stock-search-box"><input value={code} onChange={(event) => setCode(event.target.value)} placeholder="输入中文名称或股票代码，如 贵州茅台 / 600519" />{suggestions.length > 0 && <div className="stock-suggestions">{suggestions.map((item) => <button type="button" key={item.tsCode} onClick={() => void selectSuggestion(item)}><span><strong>{item.stockName}</strong><small>{item.tsCode} · {item.exchange}</small></span><em>{item.isTracked ? '已追踪' : '添加'}</em></button>)}</div>}</div><button type="submit">添加股票</button></form>{message && <div className="notice">{message}</div>}{loading ? <div className="loading-bar">正在加载追踪列表…</div> : stocks.length === 0 ? <div className="empty-module"><span className="empty-icon">◌</span><strong>还没有追踪股票</strong><span>添加一只股票，开始建立自己的观察清单。</span></div> : <div className="stock-list">{stocks.map((stock) => <article className="stock-row" key={stock.tsCode}><Link to={`/stock-tracking/${stock.tsCode}`}><div className="stock-name"><strong>{stock.stockName}</strong><span>{stock.tsCode}</span></div><Metric label="现价" value={stock.currentPrice} /><Metric label="7日涨幅" value={stock.change7dPercent} /><Metric label="PE" value={stock.peTtm} /><Metric label="PE历史百分位" value={stock.pePercentile} /><Metric label="PB" value={stock.pb} /><Metric label="PB历史百分位" value={stock.pbPercentile} /></Link><button className="text-button" onClick={() => void remove(stock.tsCode)}>移除</button></article>)}</div>}</div>;
}
